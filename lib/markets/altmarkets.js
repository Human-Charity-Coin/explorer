...
var base_url = 'https://v2.altmarkets.io/api/v2/peatio/';
  ...
  request({uri: req_url, json: true}, function (error, response, body) {
    if (error) {
      ...
      if (body.error) {
        ...
        summary['change'] = 0;
        request({ uri: base_url + 'public/markets/' + coin.toLowerCase() + exchange.toLowerCase() + `/tickers`, json: true }, function (error, response, body) {
          if (error) {
...
function get_trades(coin, exchange, cb) {
  var req_url = base_url + "/public/markets/" + coin.toLowerCase() + exchange.toLowerCase() + `/tickers/` + "&limit=50&order_by=desc";
  request({uri: req_url, json: true}, function (error, response, body) {
    if (body.error) {
      return cb(body.error, null);
    } else {
